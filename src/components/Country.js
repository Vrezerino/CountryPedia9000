...
const Country = ({ results, setResults, setErrorMsg }) => {
	if (results !== undefined && results.length > 0) {
		if (results.length > 1) {
			return (
				<div className='results'>
					{results.map(r =>
						<div key={r.name}>- {r.name}
							<button className='showCountry' key={r.name} onClick={() => setResults([r])}>show</button>
						</div>)}
				</div>
			)
			...
			return (
				<div>
					...
					<Weather capital={results[0].capital} setErrorMsg={setErrorMsg}/>
		...
		return (
			...
			Start searching for a country!
				<img className="ladybug" src="ladybug.png" alt="ladybug"/>
			</div>)
	}
}
